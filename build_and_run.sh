...
# 将 32 位汇编异常入口汇编为可链接的 ELF 目标文件。
nasm -f elf32 interrupts.asm -o build/interrupts.o

# 按 linker.ld 指定的运行地址 0x10000 链接 C 内核，生成 32 位 ELF 文件。
ld -m elf_i386 -T linker.ld -o build/kernel.elf build/kernel.o build/interrupts.o

# 去掉 ELF 文件头，只保留将被读入第 3 扇区的原始机器码。
objcopy -O binary build/kernel.elf build/kernel.bin

# 读取原始 C 内核的实际大小，避免它超过当前读取八个扇区的限制。
kernel_size=$(stat -c '%s' build/kernel.bin)

# 如果 C 内核大于 4096 字节，停止构建，防止镜像中的数据被截断。
if [ "$kernel_size" -gt 4096 ]; then
    echo "错误：kernel.bin 为 ${kernel_size} 字节，当前第 3 至第 10 扇区最多只能容纳 4096 字节。" >&2
    exit 1
fi

# 将不足 4096 字节的 C 内核末尾补 0，使它恰好占满第 3 至第 10 个磁盘扇区。
truncate -s 4096 build/kernel.bin

# 按磁盘扇区顺序拼接：第 1 扇区是启动程序，第 2 扇区是加载器，第 3 至第 10 扇区是 C 内核。
cat build/boot_sector.bin build/stage2.bin build/kernel.bin > build/cow.img

# 显示最终镜像大小；当前十个扇区应为 5120 字节。
